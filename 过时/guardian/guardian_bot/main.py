...
from guardian_bot.stack_supervisor import GuardianStackSupervisor
...
def main() -> int:
    ...
    recovery_timeout_seconds = max(60, int(runtime_cfg.get("recovery_timeout_seconds") or 180))
    heartbeat_stale_seconds = max(60, int(runtime_cfg.get("heartbeat_stale_seconds") or 240))
    heartbeat_soft_failure_limit = max(1, int(runtime_cfg.get("heartbeat_soft_failure_limit") or 1))

    supervisor = None
    if workspace_root and workspace_root.exists():
        supervisor = GuardianStackSupervisor(
            workspace_root=workspace_root,
            inspection_interval_seconds=pid_snapshot_interval,
            repair_cooldown_seconds=repair_cooldown_seconds,
            heartbeat_stale_seconds=heartbeat_stale_seconds,
            recovery_timeout_seconds=recovery_timeout_seconds,
            heartbeat_soft_failure_limit=heartbeat_soft_failure_limit,
        )
    ...
    if supervisor and auto_repair_enabled:
        try:
            startup_result = supervisor.tick(trigger="startup")
            print(
                f"[guardian] startup supervision | status={startup_result.get('status')} | action={startup_result.get('action')} | detail={str(startup_result.get('detail') or '')[:240]}",
                flush=True,
            )
        except Exception as exc:
            print(f"[guardian] startup supervision failed: {exc}", flush=True)
    ...
    while True:
        try:
            ...
            print(f"[guardian] process_pending_requests failed: {exc}", flush=True)
            summary = {"pending": 0, "approve": 0, "reject": 0, "need-info": 0, "executed": 0}
        if summary["pending"]:
            executed = summary.get("executed", 0)
        ...
        if now - last_pid_snapshot >= pid_snapshot_interval and workspace_root and workspace_root.exists():
            try:
                ...
                if supervisor and auto_repair_enabled:
                    supervision = supervisor.tick(trigger="periodic")
                    if supervision.get("status") not in {"healthy", "skipped"}:
                        print(
                            f"[guardian] supervision | status={supervision.get('status')} | action={supervision.get('action')} | detail={str(supervision.get('detail') or '')[:400]}",
                            flush=True,
                        )
            except Exception as exc:
                print(f"[guardian] periodic supervision failed: {exc}", flush=True)
            last_pid_snapshot = now
        if now - last_health_check >= health_check_interval:
            try:
                _send_periodic_inspection(cfg, workspace_root)
            except Exception as exc:
                print(f"[guardian] periodic inspection send failed: {exc}", flush=True)
