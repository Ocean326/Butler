...
def _run_repair_action(butler_root: Path) -> tuple[bool, str]:
    ...
    if guardian_manager.exists():
        try:
            ...
            return True, out or "已触发 manager.ps1 restart-stack"
...
def _build_reply(
    ...
    if repair_attempted:
        lines.append("")
        lines.append("**自动抢救动作**")
        action_label = "manager.ps1 restart-stack" if "restart-stack" in str(repair_log or "") else "manager.ps1 start butler_bot"
