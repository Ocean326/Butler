...
- Guardian 额外维护统一监督状态：`butler_main/butler_bot_code/run/guardian_stack_supervision.json`，记录最近一次巡检、恢复动作，以及对话/心跳的未完成恢复上下文
...
- 若仅 heartbeat 异常且主进程仍健康，Guardian 会优先执行 `soft-recover-heartbeat`，避免无谓打断对话主进程
- 若 `soft-recover-heartbeat` 超时或失败达到阈值，Guardian 会直接升级到 `restart-stack`；full recovery 不再走只重启主进程的旧 helper。
- 若检测到对话或 heartbeat 存在未完成上下文，Guardian 会把恢复线索写入监督状态；Butler 启动后继续沿用 recent/task ledger 进入恢复阶段，而不是把异常当作全新轮次