...
function Wait-HeartbeatSidecarHealthy {
    param(
        [string]$RunDir,
        [int]$TimeoutSeconds = 45
    )
...
function Soft-Recover-HeartbeatSidecar {
    $paths = Get-ButlerPaths
    $pythonExe = Get-ButlerPythonExecutable -BodyRoot $paths.BodyRoot

    Write-Host "[guardian] soft recover: stop heartbeat sidecar"
    Stop-HeartbeatSidecar -RunDir $paths.ButlerRunDir

    Write-Host "[guardian] soft recover: start heartbeat sidecar"
    try {
        ...
        throw "soft recover heartbeat failed: $msg"
    }
    Write-Host "[guardian] soft recover heartbeat launcher pid=$hbLauncherPid"

    Write-Host "[guardian] soft recover: verify heartbeat"
    if (-not (Wait-HeartbeatSidecarHealthy -RunDir $paths.ButlerRunDir -TimeoutSeconds 90)) {
        throw "soft recover heartbeat failed: heartbeat state not healthy within timeout"
    }
    Write-Host "[guardian] soft recover completed: heartbeat is healthy"
}
...
function Get-GuardianWorkerProcess {
    try {
        $paths = Get-ButlerPaths
    } catch {
        return $null
    }
    $statePath = Join-Path $paths.ButlerRunDir 'guardian_stack_supervision.json'
    if (-not (Test-Path $statePath)) {
        return $null
    }
    try {
        $state = Get-Content $statePath -Raw -Encoding UTF8 | ConvertFrom-Json
        $workerPid = [int]$state.guardian_pid
    } catch {
        return $null
    }
    if ($workerPid -le 0) {
    ...
    try {
        return Get-Process -Id $workerPid -ErrorAction Stop
    } catch {
        return $null
    }
}
...
function Get-GuardianProcess {
    if (-not (Test-Path $PidFile)) {
        $worker = Get-GuardianWorkerProcess
        if ($worker) {
            $worker.Id | Out-File -FilePath $PidFile -Encoding ascii -NoNewline
            return $worker
        }
        return $null
    }
    ...
    try {
        return Get-Process -Id $pidVal -ErrorAction Stop
    } catch {
        $worker = Get-GuardianWorkerProcess
        if ($worker) {
            $worker.Id | Out-File -FilePath $PidFile -Encoding ascii -NoNewline
...
function Start-Guardian {
    ...
    $argList = @('-m', 'guardian_bot.main', '--config', $ConfigPath, '--loop')
    $process = Start-Process -FilePath $PythonExe -ArgumentList $argList -WorkingDirectory $RootDir -WindowStyle Hidden -PassThru -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog
...
switch ($action) {
    ...
    'soft-recover-heartbeat' { Soft-Recover-HeartbeatSidecar }
    'status' { Show-Status }
