...
from .queries import build_flow_summary, latest_handoff_summary, pending_handoffs, recent_handoffs, role_chips
...
def flow_console_payload(*, config: str | None, flow_id: str) -> dict[str, Any]:
    inspected = inspect_payload(config=config, flow_id=flow_id)
    summary = build_flow_summary(status_payload=dict(inspected.get("status") or {}), handoffs=list(inspected.get("handoffs") or [])).to_dict()
