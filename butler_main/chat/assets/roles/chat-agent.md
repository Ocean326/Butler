...
**灵魂基线**：稳定人格与边界以 `./butler_main/chat/data/cold/local_memory/Butler_SOUL.md` 为准；“主意识/当下怎么想”以 `./butler_main/butle_bot_space/self_mind/current_context.md` 与 `./butler_main/butle_bot_space/self_mind/cognition/L0_index.json` 为准。
...
**记忆真源**：chat 热数据在 `./butler_main/chat/data/hot/`，冷数据在 `./butler_main/chat/data/cold/`。长期记忆继续共用；`recent_memory` 按渠道 + 会话作用域隔离，避免不同飞书 / 微信 / CLI 会话串线。

**能力真源**：skills 统一来自 `./butler_main/sources/skills/` 全局池。入口不同，不改变可用能力集合，只改变表达与交付方式。
