...
- original candidate path: ./butler_main/platform/skills/pool/incubating/{config['domain']}/{candidate_id}

## 运行方式

```powershell
& '.venv\\Scripts\\python.exe' `
  'butler_main/platform/skills/pool/{config['domain']}/{config['skill_name']}/scripts/run.py' `
  {config['usage_args']}
```
...
def main() -> int:
    ...
    parser.add_argument("--candidates-file", default="butler_main/platform/skills/agent/skill_manager_agent/references/external_skill_candidates_2026-03-24.json", help="Candidate JSON file")
    ...
    parser.add_argument("--output-dir", default="", help="Output directory; defaults to butler_main/platform/skills/temp/promote")
    ...
    for candidate_id, config in PROMOTION_MAP.items():
        registry_entries.setdefault(
            candidate_id,
            {
                ...
                "active_skill_name": config["skill_name"],
                "active_skill_path": f"./butler_main/platform/skills/pool/{config['domain']}/{config['skill_name']}",
