...
class OrchestratorWorkflowVMTests(unittest.TestCase):
    ...
    def test_workflow_vm_routes_research_nodes_to_research_bridge(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            boundary = vm_events[0]["payload"]["boundary"]
            self.assertEqual(boundary["vm_role"], "dispatch_router")
            self.assertEqual(boundary["execution_owner"], "research")
    ...
    def test_runner_auto_execute_can_use_workflow_vm(self) -> None:
        with self._workspace() as tmp:
            ...
            boundary = vm_events[0]["payload"]["boundary"]
            self.assertEqual(boundary["control_plane"], "orchestrator")
            self.assertEqual(boundary["execution_owner"], "runtime_os.agent_runtime")
            self.assertEqual(boundary["collaboration_owner"], "runtime_os.process_runtime")
            self.assertEqual(boundary["governance_target_owner"], "runtime_os.process_runtime")
