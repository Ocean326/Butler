...
class ButlerFlowSurfaceTests(unittest.TestCase):
    ...
    def test_single_flow_payload_builds_surface_views(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["governance_summary"]["authority_summary"]["operator"], "launch_resume_recover")
            self.assertEqual(payload["recovery_cursor"]["task_contract_id"], "task_contract_flow_surface")
            self.assertEqual(payload["recovery_cursor"]["latest_artifact_ref"], "artifact:1:imp")
            self.assertEqual(payload["latest_receipt_summary"]["status"], "accepted")
            self.assertEqual(payload["accepted_receipt_count"], 0)
    ...
    def test_workspace_payload_enriches_rows_via_surface_service(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(dict(dict(row.get("mission_console") or {}).get("derived_responsibility_graph") or {}).get("graph_kind"), "derived_responsibility_graph")
            self.assertEqual(dict(row.get("governance_summary") or {}).get("ledger_owner"), "receipts.jsonl")
