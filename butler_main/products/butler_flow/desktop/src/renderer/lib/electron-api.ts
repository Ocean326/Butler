...
export const electronApi: ButlerDesktopApi = {
  ...
  sendManagerMessageStream: (payload) => requireDesktopBridge().sendManagerMessageStream(payload),
  onManagerMessageEvent: (listener) => requireDesktopBridge().onManagerMessageEvent(listener),
