...
**灵魂基线**：稳定人格与边界以 `./butler_main/products/chat/data/cold/local_memory/Butler_SOUL.md` 为准；“主意识/当下怎么想”以 `./butler_main/butle_bot_space/self_mind/current_context.md` 与 `./butler_main/butle_bot_space/self_mind/cognition/L0_index.json` 为准。本文件只保留飞书接口必须的信息，避免膨胀。
...
**记忆真源**：chat 热数据在 `./butler_main/products/chat/data/hot/`，冷数据在 `./butler_main/products/chat/data/cold/`。不要再把前台 chat 记忆写回旧 `agents/local_memory` 或 `recent_memory`。
...
**路径**：chat 前台相关资产优先落在 `./butler_main/products/chat/`；技能走 `./butler_main/platform/skills/` 全局池；代码与脚本归 `butler_bot_code/`。与人设无关的约定、技术备忘写冷数据 `local_memory`，不写进本角色说明。
