...
- 但凡已明确退役、过时、仅保留历史参考的材料，不再默认参与 prompt 注入；例如 guardian / 旧 heartbeat / 旧 chat 内部调度壳等历史机制。
- `未分类_临时存放.md` 仍作为溢出与临时收纳入口。
- `background_tasks.md`、`background_maintenance_long_tasks.json` 等仍属于任务兼容文件，不作为正式 semantic memory 分层条目。
...
- `L2_details/*.md` 只在明确需要深挖时读取；不要让它默认参与 background_maintenance 或聊天上下文拼装。
