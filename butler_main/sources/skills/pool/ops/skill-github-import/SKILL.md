...
- 默认导入到 `./butler_main/sources/skills/pool/imported/`。
...
- `./butler_main/sources/skills/pool/ops/skill-github-import/scripts/import_github_skill.py`
...
2. 运行导入脚本，把目录下载到 `./butler_main/sources/skills/pool/imported/<skill-name>/`。
3. 检查导入目录中的：
   - `SKILL.md`
   - `scripts/`
...
4. 查看脚本自动生成的：
   ...
   - `IMPORT_REPORT.md`
5. 如果要给运行时暴露，再去更新 `./butler_main/sources/skills/collections/registry.json`。
...
- `--dest-root`：目标根目录，默认 `./butler_main/sources/skills/pool/imported`
- `--name`：可选，指定导入后的目录名
- `--replace`：若目标目录已存在则覆盖
