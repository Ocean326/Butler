...
> 状态说明（2026-03-25）：这是 `guardian` 仍被视为现役组件时的历史设计稿。当前现行链路已不再包含独立 guardian；文中的 `heartbeat` 也应理解为当时的旧后台自动化链路，而不是当前入口。
>
> 当前治理与运行边界请优先参考：`README.md`、`docs/README.md`、`docs/daily-upgrade/0325/00_当日总纲.md`、`docs/daily-upgrade/0325/05_Orchestrator完善设计与实施计划.md`。
