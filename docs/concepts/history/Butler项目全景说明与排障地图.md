...
> 状态说明（2026-03-25）：本文主体记录的是 2026-03 中旬以前的 `heartbeat / guardian / sidecar` 时代排障地图，已不再代表现行运行结构。
>
> 当前现行口径请改读：`chat core 主进程 + orchestrator 后台调度 + runtime_os/agents_os substrate + recent memory scope`。
>
> 当前入口优先级：
> 1. `README.md`
> 2. `docs/README.md`
> 3. `docs/daily-upgrade/0325/00_当日总纲.md`
> 4. `docs/daily-upgrade/0325/05_Orchestrator完善设计与实施计划.md`
> 5. `docs/daily-upgrade/0325/08_第四层接口冻结_V1_简化版.md`

## 0. 当前替代口径

1. 当前 Butler 的前台入口是 `chat`，不是旧 `heartbeat` sidecar。
2. 当前后台推进与任务控制以 `orchestrator` 为准，不再以 `heartbeat_service_runner.py` 或 watchdog 作为现行主链。
3. 当前共享执行底座以 `runtime_os / agents_os` 为准，recent memory 已按 `session_scope_id` 隔离。
4. 如果你现在在排障，优先看：
...
- `butler_bot_agent/agents/heartbeat-planner-agent.md`
- `butler_bot_agent/agents/heartbeat-planner-context.md`
- `butler_bot_agent/agents/heartbeat-planner-prompt.md`
...
- `prompts/`：运行时提示词模板与 JSON 契约，早期心跳规划模板仍保留在 `heart_beat.md` 作为兼容镜像，最新规划规则以 `butler_bot_agent/agents/heartbeat-planner-*.md` 为主
- `butler_bot/`：核心实现
- `tests/`：回归保护，尤其覆盖心跳、守护、recent memory
- `run/`：PID 和运行时状态文件
- `logs/`：主进程、心跳、错误日志
- `./butler_bot_agent/agents/recent_memory/`：最近上下文、心跳快照、legacy 短期任务镜像
- `./butler_bot_agent/agents/local_memory/`：长期偏好、约定、反思、长期任务结构化实现层
...
flowchart LR
    ...
    Guardian --> RunState
    Guardian --> Logs
    Guardian --> Reports[工作区/Butler/governance/upgrade/self_upgrade_reports]
```
...
- `memory_manager.py`
  ...
  - 维护心跳任务结构化状态（`agents/state/task_ledger.json`）与兼容镜像（`heart_beat_memory.json` / `heartbeat_long_tasks.json`）
...
- 结构化真源：`./butler_bot_agent/agents/state/task_ledger.json`
- planner 文本读口：`./butler_bot_agent/agents/local_memory/heartbeat_tasks.md`
...
更完整的「新陈代谢 / 统一记忆」设计与说明见：

- `docs/heartbeat_新陈代谢机制_试运行_20260310.md`
- `docs/统一记忆机制_说明文档_20260310.md`

当这几份设计文档与本节存在细微出入时，以**更新时间更近的文档与实际运行信号**为准，并优先遵守其中对「当前真源层级 / 信息新旧权重」的约定。
...
| `butler_bot_agent/agents/heartbeat-planner-prompt.md` | 心跳规划提示词主入口 | 想改规划思路与分支结构时优先看这里；`prompts/heart_beat.md` 仅保留 JSON 契约兼容 |
...
2. `记忆系统真源说明.md`：把 recent / local / task_ledger / heartbeat_tasks / heart_beat_memory / heartbeat_long_tasks 的关系画成一张图

（2026-03-11 补充：当前已存在 `docs/统一记忆机制_说明文档_20260310.md` 与 `docs/self_mind与回忆机制_总体设计_20260311.md`，部分承担了这份“记忆系统真源说明”的角色；后续若仍需要单独文档，可重点从“运行真源索引与版本图”视角补充，而不是重复现有内容。）
