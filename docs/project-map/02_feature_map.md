...
- 关键代码目录：`butler_main/products/chat/`（canonical physical）、`butler_main/products/campaign_orchestrator/console/`（canonical physical）、`butler_main/products/campaign_orchestrator/orchestrator/interfaces/`（canonical control surface）、`butler_main/chat/`（compat shell）、`butler_main/console/`（compat shell）
- 当前真源文档：
...
- 关键代码目录：`butler_main/products/campaign_orchestrator/orchestrator/`（canonical physical）、`butler_main/products/campaign_orchestrator/campaign/`（canonical physical）、`butler_main/orchestrator/`（compat shell）、`butler_main/domains/campaign/`（compat shell）
- 当前真源文档：
  - [0331 后台主线 Campaign 宏账本与 Agent 可重入内环实施回写（历史文件名保留草稿）](../daily-upgrade/0331/03_后台主线控制面瘦身与Agent内环提权草稿计划.md)
...
- 主层级：`Product Surface（产品表面层）`
- 次层级：`Domain & Control Plane（领域与控制平面）`
- 关键代码目录：`butler_main/products/campaign_orchestrator/orchestrator/`（canonical physical）、`butler_main/products/campaign_orchestrator/orchestrator/interfaces/query_service.py`（canonical query surface）、`butler_main/products/campaign_orchestrator/orchestrator/feedback_notifier.py`（canonical feedback surface）、`butler_main/products/chat/`（canonical physical）、`butler_main/chat/task_query.py`（compat query entry）
- 当前真源文档：
  - [0331 后台主线 Campaign 宏账本与 Agent 可重入内环实施回写（历史文件名保留草稿）](../daily-upgrade/0331/03_后台主线控制面瘦身与Agent内环提权草稿计划.md)
...
- 关键代码目录：`butler_main/platform/runtime/`（canonical package entry）、`butler_main/runtime_os/multi_agent_runtime/`（physical runtime）、`butler_main/runtime_os/process_runtime/session/`（physical session runtime）、`butler_main/compat/multi_agents_os/`（compat package entry）、`butler_main/multi_agents_os/session/`（legacy compat implementation）
- 当前真源文档：
  - [系统分层与事件契约](../runtime/System_Layering_and_Event_Contracts.md)
...
- 次层级：`Domain & Control Plane（领域与控制平面）`
- 关键代码目录：`butler_main/platform/runtime/`（canonical package entry）、`butler_main/runtime_os/multi_agent_protocols/`（physical protocol layer）、`butler_main/runtime_os/process_runtime/templates/`（compat template layer）
- 当前真源文档：
  - [系统分层与事件契约](../runtime/System_Layering_and_Event_Contracts.md)
  - [Workflow IR 正式口径](../runtime/WORKFLOW_IR.md)
- 改前读包：`runtime-contract`
...
- 次层级：`Domain & Control Plane（领域与控制平面）`
- 关键代码目录：`butler_main/platform/runtime/`（canonical package entry）、`butler_main/runtime_os/durability_substrate/`（physical durability layer）、`butler_main/runtime_os/process_runtime/engine/`（compat engine layer）、`butler_main/products/campaign_orchestrator/orchestrator/workflow_ir.py`（canonical workflow IR entry）
- 当前真源文档：
  - [系统分层与事件契约](../runtime/System_Layering_and_Event_Contracts.md)
  - [Workflow IR 正式口径](../runtime/WORKFLOW_IR.md)
- 改前读包：`runtime-contract`
...
- 关键代码目录：`butler_main/platform/skills/`（canonical physical）、`butler_main/compat/agents_os/`（compat package entry）、`butler_main/agents_os/skills/`（compat runtime hook）、`butler_main/sources/skills/`（compat namespace only）、`butler_main/products/chat/`（canonical physical）、`butler_main/chat/`（compat shell）、`butler_main/products/campaign_orchestrator/campaign/`（canonical physical）、`butler_main/domains/campaign/`（compat shell）
- 当前真源文档：
...
- 关键代码目录：`butler_main/products/butler_flow/`（canonical physical）、`butler_main/butler_flow/`（compat shell）、`butler_main/platform/runtime/`（canonical package entry）、`butler_main/runtime_os/agent_runtime/`（physical agent runtime）、`tools/butler-flow`、`butler_main/butler_cli.py`
- 当前真源文档：
  - [当前系统基线](./00_current_baseline.md)
...
- 关键代码目录：`butler_main/products/campaign_orchestrator/`（canonical physical）、`butler_main/platform/runtime/`（canonical package entry）、`butler_main/compat/agents_os/`（compat package entry）、`butler_main/compat/multi_agents_os/`（compat package entry）、`butler_main/products/campaign_orchestrator/orchestrator/framework_*`（canonical framework mapping）、`butler_main/runtime_os/`（physical runtime）、`butler_main/agents_os/`（compat runtime）、`butler_main/multi_agents_os/`（compat runtime）、`butler_main/products/campaign_orchestrator/console/`（canonical console surface）、`butler_main/products/chat/`（canonical physical）、`butler_main/chat/`（compat shell）
- 当前真源文档：
