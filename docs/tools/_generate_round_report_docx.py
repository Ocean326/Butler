...
items = [
    ...
    "配置与文档同步：butler_bot.json(.example) 与 README 已更新模型、后台自动化、记忆 CLI 的使用说明。",
]
for i in items:
    doc.add_paragraph(i, style="List Bullet")

doc.add_heading("二、后台自动化机制", level=2)
items = [
    ...
    "发送路径：复用飞书私聊发送能力，支持 background.receive_id / receive_id_type，未配置时可回退启动通知目标。",
    "可扩展能力：支持 background.agent_prompt 先调用模型生成后台自动化消息，再发送。",
]
for i in items:
    doc.add_paragraph(i, style="List Bullet")
...
issues = [
    "问题：后台自动化首次延迟和后续周期可能被混用。",
    "定位：无状态计算会导致每轮都使用 startup_delay_seconds。",
    "解决：引入 background bootstrap 状态，首次走 startup_delay_seconds，后续固定走 every_minutes。",
    "验证：单测通过（test_background_uses_startup_delay_once_then_interval）；启动日志出现“[后台自动化] 已启动（单次计时器重挂模式）”。",
]
for i in issues:
    doc.add_paragraph(i, style="List Bullet")
...
items = [
    ...
    "私聊发送通用化：抽出 _send_private_message，启动通知和后台自动化复用。",
]
for i in items:
    doc.add_paragraph(i, style="List Bullet")
