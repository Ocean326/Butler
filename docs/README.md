...
- 补充：`0407/02` 当前已回写 `P0-P3` 的代码落地结果：`task_contract.json`、`receipts.jsonl`、`recovery_cursor.json`，以及 `task_contract_id + latest_receipt_summary + recovery_state`
- 补充：`0407/02` 当前也已回写 `P4-P5` 的完成态：typed governance receipt、authority/policy 写路径硬化、`surface_meta / mission_console / derived_responsibility_graph`，以及可见壳层正式收口为 `Mission Index / Contract Studio / Run Console`
- 补充：`0407/02` 当前已进入 `P6` gate 证据封板阶段；旧 `workspace / history / flows / manage_center / single_flow` 只保 compat alias，不再作为显示标题
- 补充：[0407 vibe-close：docs 默认并回 `main` 与 worktree 整理策略](./daily-upgrade/0407/03_vibe-close_docs默认并回main与worktree整理策略.md)
...
- 补充：若目标已进入 Butler Desktop 实装、Electron 壳、Python bridge、shared surface 共用投影、`Config Path Fallback`、或运行验证拆层，先补读 [0403 Butler Flow Desktop 壳与 shared surface bridge 落地](./daily-upgrade/0403/03_butler-flow_desktop壳与shared_surface_bridge落地.md)；若目标是前端信息架构、Manager 线程、agent 子流展示与产品叙事，再补读 [0408 Butler Desktop 产品形态收口：Manager 对话壳、连续线程与多藏 agent 输出](./daily-upgrade/0408/01_butler-desktop产品形态收口_manager对话壳_连续线程与多藏agent输出.md)。
