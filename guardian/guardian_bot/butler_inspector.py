...
def inspect_butler_main(butler_root: Path) -> dict:
    ...
    if hb_state_path.exists():
        try:
            ...
            alive = _is_pid_alive(active_pid)
            running = result["heartbeat"]["state"] == "running"
            ...
            if recent_watchdog and run_updated_at is not None:
                ...
                if run_age > 420 and run_state == "running" and run_phase in {"plan", "execute"}:
    ...
    parts.append(f"守护职责: {g['state']} {'✓' if g['healthy'] else '✗'}")
...
def format_inspection_report(inspection: dict) -> str:
    ...
    lines = [
        ...
        f"- 守护职责: {inspection['guardian_handover']['state']} ({inspection['guardian_handover']['detail']})",
    ]
